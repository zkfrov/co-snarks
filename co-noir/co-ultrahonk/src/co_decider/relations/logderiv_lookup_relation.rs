...
impl LogDerivLookupRelation {
    ...
    // Compute table_1 + gamma + table_2 * eta + table_3 * beta_sqr + table_4 * beta_cube
    ...
    for LogDerivLookupRelation
{
     ...
     *
     * where write_term = table_col_1 + \gamma + table_col_2 * \beta + table_col_3 * \beta_sqr + table_index * \beta_cube
     * and read_term = derived_table_entry_1 + \gamma + derived_table_entry_2 * \beta + derived_table_entry_3 * \beta_sqr
     * + table_index * \beta_cube, with derived_table_entry_i = w_i - col_step_size_i\cdot w_i_shift. (The table entries
